...
const ACTemperature: React.FC<Props> = ({ data, frequency }) => {
  const n = useRef(9);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      _data?.splice(0, 1);
      _data?.push(data[n.current].ac_temperature);
    ...
    <Line
      ...
      data={{
        ...
        datasets: [
          {
            type: "line",
            label: `AC Temperature in Celcius`,
            data: toShow?.map((el) => el.ac_temperature),
