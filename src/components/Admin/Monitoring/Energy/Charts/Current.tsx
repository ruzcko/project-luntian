...
const SolarCurrent: React.FC<Props> = ({ data, frequency }) => {
  const n = useRef(9);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      _data?.splice(0, 1);
      _data?.push(data[n.current]?.solar_current);
    ...
    <Line
      ...
      data={{
        ...
        datasets: [
          {
            type: "line",
            label: `Solar Current (Ampere)`,
            data: toShow?.map((el) => el.solar_current),
