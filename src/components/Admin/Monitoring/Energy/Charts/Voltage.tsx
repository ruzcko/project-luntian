...
const SolarVoltage: React.FC<Props> = ({ data, frequency }) => {
  const n = useRef(9);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      _data?.splice(0, 1);
      _data?.push(data[n.current]?.solar_voltage);
    ...
    <Line
      ...
      data={{
        ...
        datasets: [
          {
            type: "line",
            label: `Solar Voltage (Volt)`,
            data: toShow?.map((el) => el.solar_voltage),
