...
const PondNitrite: React.FC = () => {
  ...
  useEffect(() => {
    csv("/data/aquaculture/pond_nitrite.csv", (_): PDoutput => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      bar?.splice(0, n - bars);
      bar?.push(Math.floor(Math.random() * 5));
