...
const PondTotalSolids: React.FC<Props> = ({ data, frequency }) => {
  const n = useRef(9);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      _data?.splice(0, 1);
      _data?.push(data[n.current].pond_total_dissolved_solids);
        ...
        datasets: [
          {
            ...
            data: toShow?.map((el) => el.pond_total_dissolved_solids),
